...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        List<String> itemListOnSale = shopStream.flatMap(x -> x.getItemList().stream().map(Item::getName)).collect(Collectors.toList());
        Set<String> itemSetNotOnSale = customerStream
                .flatMap(x -> x.getWantToBuy().stream())
                .map(Item::getName)
                .filter(itemName -> itemListOnSale
                        .stream()
                        .noneMatch(itemName::equals))//co tu się dokładnie dzieje?
    ...
    public void havingEnoughMoney() {
         ...
         */
        List<Item> onSale = shopStream.flatMap(x -> x.getItemList().stream()).collect(Collectors.toList());
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(
                wantedItem -> onSale.stream().filter(shopItem -> shopItem.getName().equals(wantedItem.getName()))
                        ...
                        .findFirst().map(Item::getPrice).get())//co tutaj robi orElse?
                .sum();
